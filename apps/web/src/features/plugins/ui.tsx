/**
 * Plugin governance surface primitives.
 *
 * Local to `features/plugins/**`, following the same precedent as
 * `features/identity/ui.tsx`, `features/webhooks/ui.tsx`, and
 * `features/data-governance/ui.tsx`: the shared design-system components under
 * `src/components/ui` are not generated in this repository, so each surface owns
 * primitives built from the semantic tokens in `src/styles/globals.css`. Nothing
 * here introduces a palette, typeface, radius, or decorative style.
 */
...
/**
 * A horizontal tab strip for the sub-pages of one surface.
 *
 * Roving tabindex WITH arrow-key movement, following the ARIA tabs pattern and
 * the precedent in `features/data-governance/ui.tsx` and
 * `features/identity/ui.tsx`. The combination matters: roving tabindex on its own
 * would make every unselected tab unreachable from the keyboard, because a
 * `tabindex="-1"` button can only be reached programmatically.
 */
 ...
 * pattern the other P06/P07 surfaces use. A block, an unblock, and a pin all
 * demand a reason, because the server refuses an empty one and the reason is what
 * makes the action auditable months later.
 */
