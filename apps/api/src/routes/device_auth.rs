...
use crate::{
    ...
    repositories::{
        DeviceAuthorizationRepository, IdentityRepository, SecurityEventInput,
...
pub async fn start(
    ...
    let device_name = validate_device_label(&body.device_name).map_err(|_| {
        validation_error(
            &context,
            "device_name_invalid",
    ...
    let code_challenge = validate_pkce_challenge(&body.code_challenge).map_err(|_| {
        validation_error(&context, "pkce_invalid", "The PKCE challenge is invalid.")
    })?;
    ...
    let device_code_hash = sha256_hex(&device_code)
        .await
        .map_err(|_| service_unavailable(&context))?;
    let expires_at = add_seconds(&context.received_at, DEVICE_TTL_SECONDS)
        .map_err(|_| service_unavailable(&context))?;
...
pub async fn exchange(
    ...
    let device_hash = sha256_hex(&body.device_code)
        .await
        .map_err(|_| service_unavailable(&context))?;
