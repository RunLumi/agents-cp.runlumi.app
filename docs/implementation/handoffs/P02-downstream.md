...
- Authorization: default roles are `owner`, `admin`, `member`, `viewer`; stable P02 permissions are `org.read`, `org.manage`, `org.lifecycle`, `org.ownership_transfer`, `members.read`, `members.manage`, `teams.read`, `teams.manage`, and `audit.read`. Unknown/stale/cross-org decisions deny.
...
- Apply `0002_p02_identity_organizations.sql` then `0003_p02_auth_rate_limits.sql` before P03 migrations.
- D1 `DB` and `OUTBOX_QUEUE` bindings are already configured. P02 does not require a new secret.
